...
switch ( $action ) {
	...
	case 'confirm_admin_email':
		 ...
		 * as the current user is not set, see sr-includes/pluggable.php.
		...
		<form class="admin-email-confirm-form" name="admin-email-confirm-form" action="<?php echo esc_url( site_url( 'sr-login.php?action=confirm_admin_email', 'login_post' ) ); ?>" method="post">
			<?php
			/**
			...
			<p class="admin-email__details">
				...
				$admin_email_help_url = __( '/documentation/article/settings-general-screen/#email-address' );
	...
	case 'retrievepassword':
		if ( $http_post ) {
			...
			if ( ! is_wp_error( $errors ) ) {
				$redirect_to = ! empty( $_REQUEST['redirect_to'] ) ? $_REQUEST['redirect_to'] : 'sr-login.php?checkemail=confirm';
				wp_safe_redirect( $redirect_to );
				exit;
			}
		}
		...
		<form name="lostpasswordform" id="lostpasswordform" action="<?php echo esc_url( network_site_url( 'sr-login.php?action=lostpassword', 'login_post' ) ); ?>" method="post">
			<p>
		...
		<p id="nav">
			...
			if ( get_option( 'users_can_register' ) ) {
				...
				/** This filter is documented in sr-includes/general-template.php */
	...
	case 'rp':
		...
		if ( ! $user || is_wp_error( $user ) ) {
			...
			if ( $user && $user->get_error_code() === 'expired_key' ) {
				wp_redirect( site_url( 'sr-login.php?action=lostpassword&error=expiredkey' ) );
			} else {
				wp_redirect( site_url( 'sr-login.php?action=lostpassword&error=invalidkey' ) );
			}
		...
		?>
		<form name="resetpassform" id="resetpassform" action="<?php echo esc_url( network_site_url( 'sr-login.php?action=resetpass', 'login_post' ) ); ?>" method="post" autocomplete="off">
	...
	case 'register':
		if ( is_multisite() ) {
			 ...
			 */
			wp_redirect( apply_filters( 'wp_signup_location', network_site_url( 'sr-signup.php' ) ) );
			exit;
		}

		if ( ! get_option( 'users_can_register' ) ) {
			wp_redirect( site_url( 'sr-login.php?registration=disabled' ) );
			exit;
		}
		...
		if ( $http_post ) {
			...
			if ( ! is_wp_error( $errors ) ) {
				$redirect_to = ! empty( $_POST['redirect_to'] ) ? $_POST['redirect_to'] : 'sr-login.php?checkemail=registered';
				wp_safe_redirect( $redirect_to );
				exit;
			}
		}
		...
		?>
		<form name="registerform" id="registerform" action="<?php echo esc_url( site_url( 'sr-login.php?action=register', 'login_post' ) ); ?>" method="post" novalidate="novalidate">
			<p>
		...
		<p id="nav">
			...
			/** This filter is documented in sr-login.php */
	...
	case 'checkemail':
		...
		/** This action is documented in sr-login.php */
	...
	default:
		...
		if ( isset( $_REQUEST['redirect_to'] ) ) {
			...
			if ( $secure_cookie && str_contains( $redirect_to, 'sr-admin' ) ) {
			...
			if ( headers_sent() ) {
				$user = new WP_Error(
					...
					sprintf(
						...
						__( '/documentation/article/cookies/' ),
						__( '/support/forums/' )
					)
				);
				...
				$user = new WP_Error(
					...
					sprintf(
						...
						__( '/documentation/article/cookies/#enable-cookies-in-your-browser' )
					)
				);
			}
		}
		...
		if ( ! is_wp_error( $user ) && ! $reauth ) {
			...
			if ( ( empty( $redirect_to ) || 'sr-admin/' === $redirect_to || admin_url() === $redirect_to ) ) {
		...
		if ( $interim_login ) {
			...
			} elseif ( isset( $_GET['redirect_to'] ) && str_contains( $_GET['redirect_to'], 'sr-admin/authorize-application.php' ) ) {
		...
		<form name="loginform" id="loginform" action="<?php echo esc_url( site_url( 'sr-login.php', 'login_post' ) ); ?>" method="post">
			<p>
				<label for="user_login"><?php _e( 'Username or Email Address' ); ?></label>
