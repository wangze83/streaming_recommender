...
if(!function_exists('doo_logo_admin')){
    function doo_logo_admin() {
    	$logo = (dooplay_get_option('adminloginlogo')['url']) ? doo_compose_image_option('adminloginlogo') : DOO_URI ."/assets/img/brand/logo_blue.svg";
...
function doo_cast_json($name, $type, $limit = false) {
    if ($type == "img") {
        if ($limit) {
            $val    = explode("]", $name);
            $passer = $newvalor = array();
...
# Get director
function doo_director_json($name, $type, $limit = false) {
    if ($type == "img"){
        if ($limit) {
            $val    = explode("]", $name);
            $passer = $newvalor = array();
...
function doo_li_genres_without_html()
{
    $transient = get_transient('dooplay_genres_widget_without_html');
    if (false === $transient) {
        $args = array(
            'post_type' => '',
            'taxonomy' => 'genres',
        ...
        return (array)wp_list_categories_without_html($args);
    }
}
...
function doo_login_form(){
    ...
    $action = esc_url( site_url('sr-login.php', 'login_post') );
    $lostpassword = esc_url( site_url('sr-login.php?action=lostpassword', 'login_post') );
    $form = '
    <div class="login_box">
        <div class="box">
