...
add_action('wp_ajax_genres_action', 'genres_action_callback');
add_action('wp_ajax_director_action', 'director_action_callback');
add_action('wp_ajax_cast_action', 'cast_action_callback');
add_action('wp_ajax_rating_action', 'rating_action_callback');
add_action('wp_ajax_nopriv_content_based_action', 'Content_Based_action_callback');
add_action('wp_ajax_content_based_action', 'Content_Based_action_callback');
...
function starstruck_array( $data, $id ) {
    if ( ! $data ) {
        return;
    }
    $data  = maybe_unserialize( $data );
...
function genres_action_callback() {
    $id = $_POST['id'];
    return get_the_term_list_json($id, 'genres', '', '', '');

}
function director_action_callback() {
    $id = $_POST['id'];
    $postmeta = doo_postmeta_movies($id);
    $r = doo_director_json(doo_isset($postmeta,'dt_dir'), "img", true);
    echo json_encode($r);
    die;

}
function cast_action_callback() {
    $id = $_POST['id'];
    $postmeta = doo_postmeta_movies($id);
    $r = doo_cast_json(doo_isset($postmeta,'dt_cast'), "img", true);
    echo json_encode($r);
    die;

}
function rating_action_callback() {
    $id = $_POST['id'];
    if ( ! isset( $_POST['type'] ) ) {
        $type = 'post';
    } else {
    ...
    $response = starstruck_array( $data, $id );
...
function getContentBasedCF_recommendations($title = 'Ice Age: Continental Drift')
{
    $url = 'http://flask-api:5000/content_based_CF_recommendations';
    $data = json_encode(['movie' => $title]);
    $ch = curl_init($url);
...
function Content_Based_action_callback() {
    $title = $_REQUEST['title'];
    $genre = 'comedy';
    $res = getContentBasedCF_recommendations($title);

    $transient = new WP_Query(array(
        'post_titles' => $res['recommendations'],
    ));
    ...
    ?>
        <?php while ($transient->have_posts()) : $transient->the_post(); ?>
            <div class="owl-item" style="width: 201px;">
    ...
    header('Content-Type: application/json');
    echo json_encode(array('html' => $content, 'recommendations' => $res['recommendations']));
    die();
}

function getTopMovieIds($limit=20, $rangt = 20, $order = 'DESC', $mvote = 200){
...
//	}
    ...
    $logFilePath = '/var/www/html/log/rating.log';
...
function getMysqlData($sql) {
    ...
    $username = "root";
    $password = "123456";
    $dbname = "streaming_recommender_db";
