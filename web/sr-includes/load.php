...
function wp_check_php_mysql_versions() {
	...
	if ( version_compare( $required_php_version, $php_version, '>' ) ) {
		...
		printf(
			'Your server is running PHP version %1$s but  %2$s requires at least %3$s.',
			$php_version,
			$wp_version,
	...
	if ( ! function_exists( 'mysqli_connect' ) && ! function_exists( 'mysql_connect' )
		...
		$message = '<p>' . __( 'Your PHP installation appears to be missing the MySQL extension which is required by .' ) . "</p>\n";
 ...
 *
 * The development mode affects how certain parts of the  application behave,
 * which is relevant when developing for .
 *
 ...
 * It does not affect debugging output, but rather functional nuances in .
 *
...
/**
 * Ensures all of  is not loaded when handling a favicon.ico request.
 *
...
/**
 * Sets PHP error reporting based on  debug settings.
 *
...
function wp_debug_mode() {
	 ...
	 *  loads, usually in `wp-config.php`.
	 *
	 * Example:
	 *
...
/**
 * Redirects to the installer if  is not installed.
 *
...
function wp_not_installed() {
	...
	$link = wp_guess_url() . '/sr-admin/install.php';
...
function wp_get_active_and_valid_themes() {
	...
	if ( wp_installing() && 'sr-activate.php' !== $pagenow ) {
		return $themes;
	}
...
/**
 * Determines whether  is in Recovery Mode.
 *
...
function is_protected_endpoint() {
	// Protect login pages.
	if ( isset( $GLOBALS['pagenow'] ) && 'sr-login.php' === $GLOBALS['pagenow'] ) {
		return true;
	}
 ...
 * @see wp_login_url()
 *
 * @return bool True if inside  login screen, false otherwise.
 */
function is_login() {
 ...
 * @global WP_Screen $current_screen  current screen object.
 *
 * @return bool True if inside  site administration pages.
 */
...
/**
 * Checks or sets whether  is in "installation" mode.
 *
...
function wp_is_jsonp_request() {
	...
	/** This filter is documented in sr-includes/rest-api/class-wp-rest-server.php */
