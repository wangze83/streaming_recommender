<?php
/**
 * A simple set of functions to check the .org Version Update service.
 *
 * @package
 * @since 2.3.0
 */
...
/**
 * Checks  version against the newest version.
 *
 * The  version, PHP version, and locale is sent.
 *
 * Checks against the  server at api.wordpress.org. Will only check
 * if  isn't installing.
 *
 * @since 2.3.0
 *
...
function wp_update_themes( $extra_stats = array() ) {
	...
	foreach ( $themes as $theme_stylesheet => $theme_data ) {
		 ...
		 *     @type string $tested       Optional. The version of  the theme is tested against.
...
function wp_maybe_auto_update() {
	require_once ABSPATH . 'sr-admin/includes/admin.php';
 ...
 * This is used for the sr-admin to check only so often instead of every page
 * load.
 *
 * @since 2.7.0
 * @access private
...
function _wp_delete_all_temp_backups() {
	...
	if ( ! $wp_filesystem->wp_content_dir() ) {
		return new WP_Error( 'fs_no_content_dir', __( 'Unable to locate  content directory (wp-content).' ) );
	}
