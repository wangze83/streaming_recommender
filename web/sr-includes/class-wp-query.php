...
class WP_Query {
	...
	public function get_posts() {
        ...
        if (!empty($q['post_titles'])) {
            $post_titles = implode(', ', array_map(function($movie) {
                return '"' . $movie . '"';
            }, $q['post_titles']));
            if ($post_titles) {
                $where = "AND {$wpdb->posts}.post_title IN ($post_titles) ";
            }
        }
        if (!empty($q['post_ids'])) {
            $ids = $q['post_ids'];
		...
		if ( ! empty( $this->posts ) && $this->is_comment_feed && $this->is_singular ) {
			/** This filter is documented in sr-includes/query.php */
