...
function wp_logout_url( $redirect = '' ) {
	...
	$logout_url = add_query_arg( $args, site_url( 'sr-login.php?action=logout', 'login' ) );
...
function wp_login_url( $redirect = '', $force_reauth = false ) {
	$login_url = site_url( 'sr-login.php', 'login' );
...
function wp_registration_url() {
	 ...
	 */
	return apply_filters( 'register_url', site_url( 'sr-login.php?action=register', 'login' ) );
}
...
function wp_login_form( $args = array() ) {
	...
	$form =
		sprintf(
			...
			esc_url( site_url( 'sr-login.php', 'login_post' ) )
		) .
		$login_form_top .
		sprintf(
...
function wp_lostpassword_url( $redirect = '' ) {
	...
	if ( is_multisite() ) {
		...
		$wp_login_path = $blog_details->path . 'sr-login.php';
	} else {
		$wp_login_path = 'sr-login.php';
	}
			...
			foreach ( (array) $results as $result ) {
				if ( '0000-00-00 00:00:00' !== $result->post_date ) {
					...
					if ( $result->post_title ) {
						/** This filter is documented in sr-includes/post-template.php */
 ...
 * See wp.editor.initialize() in sr-admin/js/editor.js for initialization options.
 *
 * @uses _WP_Editors
 * @since 4.8.0
 */
...
function wp_get_code_editor_settings( $args ) {
	$settings = array(
		...
		'jshint'     => array(
			// The following are copied from <https://github.com//wordpress-develop/blob/4.8.1/.jshintrc>.
			'boss'     => true,
			'curly'    => true,
			'eqeqeq'   => true,
			'eqnull'   => true,
 ...
 * @since 2.3.0
 *
 * @param string $file file relative to sr-admin/ without its ".css" extension.
 * @return string
 */
function wp_admin_css_uri( $file = 'sr-admin' ) {
 ...
 * $file is a file relative to sr-admin/ without its ".css" extension. A
 * stylesheet link to that generated URL is printed.
 *
 * @since 2.3.0
 *
 * @param string $file       Optional. Style handle name or file name (without ".css" extension) relative
 *                           to sr-admin/. Defaults to 'sr-admin'.
 * @param bool   $force_echo Optional. Force the stylesheet link to be printed rather than enqueued.
 */
function wp_admin_css( $file = 'sr-admin', $force_echo = false ) {
	 ...
	 *                                relative to sr-admin/. Defaults to 'sr-admin'.
	 */
...
function get_the_generator( $type = '' ) {
	...
	switch ( $type ) {
		...
		case 'xhtml':
			...
			break;
		case 'atom':
			$gen = '<generator uri="/" version="' . esc_attr( get_bloginfo_rss( 'version' ) ) . '"></generator>';
			break;
		case 'rss2':
			$gen = '<generator>' . sanitize_url( '/?v=' . get_bloginfo_rss( 'version' ) ) . '</generator>';
			break;
		case 'rdf':
			$gen = '<admin:generatorAgent rdf:resource="' . sanitize_url( '/?v=' . get_bloginfo_rss( 'version' ) ) . '" />';
			break;
		case 'comment':
 ...
 * @return string HTML attribute or empty string.
 */
function __checked_selected_helper( $helper, $current, $display, $type ) { // phpcs:ignore .NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
