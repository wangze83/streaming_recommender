...
function wp_authenticate_application_password( $input_user, $username, $password ) {
	...
	foreach ( $hashed_passwords as $key => $item ) {
		...
		if ( is_wp_error( $error ) && $error->has_errors() ) {
			/** This action is documented in sr-includes/user.php */
			do_action( 'application_password_failed_authentication', $error );
 ...
 * @since 6.0.0 Moved to sr-includes/user.php.
 *
...
function sanitize_user_field( $field, $value, $user_id, $context ) {
	...
	if ( 'edit' === $context ) {
		if ( $prefixed ) {
			...
			/** This filter is documented in sr-includes/post.php */
...
function retrieve_password( $user_login = null ) {
	...
	$message .= network_site_url( "sr-login.php?action=rp&key=$key&login=" . rawurlencode( $user_login ), 'login' ) . '&wp_lang=' . $locale . "\r\n\r\n";
	...
	if ( ! wp_mail( $to, $subject, $message, $headers ) ) {
		$errors->add(
			...
			sprintf(
				...
				esc_url( __( '/documentation/article/reset-your-password/' ) )
			)
		);
		return $errors;
	}
 ...
 *
 * @since 3.0.0
 * @since 4.9.0 This function was moved from sr-admin/includes/ms.php so it's no longer Multisite specific.
 *
