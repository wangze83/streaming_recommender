...
if ( ! function_exists( 'auth_redirect' ) ) :
	...
	function auth_redirect() {
		...
		if ( $secure && ! is_ssl() && str_contains( $_SERVER['REQUEST_URI'], 'sr-admin' ) ) {
		...
		if ( $user_id ) {
			...
			if ( ! $secure && get_user_option( 'use_ssl', $user_id ) && str_contains( $_SERVER['REQUEST_URI'], 'sr-admin' ) ) {
				if ( str_starts_with( $_SERVER['REQUEST_URI'], 'http' ) ) {
					wp_redirect( set_url_scheme( $_SERVER['REQUEST_URI'], 'https' ) );
					exit;
				} else {
...
if ( ! function_exists( 'wp_redirect' ) ) :
	 ...
	 */
	function wp_redirect( $location, $status = 302, $x_redirect_by = '' ) {
		global $is_IIS;
...
if ( ! function_exists( 'wp_safe_redirect' ) ) :
	 ...
	 * list.
	 *
	 * If the host is not allowed, then the redirect defaults to sr-admin on the siteurl
...
if ( ! function_exists( 'wp_new_user_notification' ) ) :
	...
	function wp_new_user_notification( $user_id, $deprecated = null, $notify = '' ) {
		...
		$message .= network_site_url( "sr-login.php?action=rp&key=$key&login=" . rawurlencode( $user->user_login ), 'login' ) . "\r\n\r\n";
