...
function redirect_canonical( $requested_url = null, $do_redirect = true ) {
	...
	/*
	 * If we're not in sr-admin and the post has been published and preview nonce
	...
	if ( is_404() ) {
		...
		if ( 'wp-register.php' === basename( $redirect['path'] ) ) {
			if ( is_multisite() ) {
				...
				$redirect_url = apply_filters( 'wp_signup_location', network_site_url( 'sr-signup.php' ) );
			} else {
 ...
 *
 * If a user visits example.com/admin, they'll be redirected to /sr-admin.
 * Visiting /login redirects to /sr-login.php, and so on.
 *
 * @since 3.4.0
 *
 * @global WP_Rewrite $wp_rewrite  rewrite component.
...
function wp_redirect_admin_locations() {
	...
	$logins = array(
		home_url( 'sr-login.php', 'relative' ),
