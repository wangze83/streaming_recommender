 ...
 *
 * Schedules a hook which will be triggered by  at the specified UTC time.
...
function wp_schedule_event( $timestamp, $recurrence, $hook, $args = array(), $wp_error = false ) {
	...
	/** This filter is documented in sr-includes/cron.php */
	$pre = apply_filters( 'pre_schedule_event', null, $event, $wp_error );
...
function spawn_cron( $gmt_time = 0 ) {
	...
	if ( defined( 'ALTERNATE_WP_CRON' ) && ALTERNATE_WP_CRON ) {
		...
		require_once ABSPATH . 'sr-cron.php';
		return true;
	}
	...
	$cron_request = apply_filters(
		'cron_request',
		array(
			'url'  => add_query_arg( 'doing_wp_cron', $doing_wp_cron, site_url( 'sr-cron.php' ) ),
			'key'  => $doing_wp_cron,
			'args' => array(
				'timeout'   => 0.01,
				'blocking'  => false,
 ...
 */
function _wp_cron() {
	// Prevent infinite loops caused by lack of sr-cron.php.
	if ( str_contains( $_SERVER['REQUEST_URI'], '/sr-cron.php' ) || ( defined( 'DISABLE_WP_CRON' ) && DISABLE_WP_CRON ) ) {
		return 0;
	}
