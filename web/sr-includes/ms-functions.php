 ...
 * at sr-admin/network/settings.php. The check is only run on
 * self-registrations; user creation at sr-admin/network/users.php
 * bypasses this check.
 *
 * @since MU (3.0.0)
 *
...
function wpmu_validate_user_signup( $user_name, $user_email ) {
	...
	/** This filter is documented in sr-includes/user.php */
...
function wpmu_signup_blog_notification( $domain, $path, $title, $user_login, $user_email, $key, $meta = array() ) {
	...
	if ( ! is_subdomain_install() || get_current_network_id() != 1 ) {
		$activate_url = network_site_url( "sr-activate.php?key=$key" );
	} else {
		$activate_url = "http://{$domain}{$path}sr-activate.php?key=$key"; // @todo Use *_url() API.
	}
...
function wpmu_signup_user_notification( $user_login, $user_email, $key, $meta = array() ) {
	...
	$message         = sprintf(
		...
		site_url( "sr-activate.php?key=$key" )
	);
...
Username: USERNAME
Password: PASSWORD
Log in here: BLOG_URLsr-login.php
 ...
 * which is defined in sr-includes/functions.php in
 * get_allowed_mime_types(). This function is used to filter
 * that list against the filetypes allowed provided by Multisite
 * Super Admins at sr-admin/network/settings.php.
 *
 * @since MU (3.0.0)
 *
 * @param array $mimes
...
function signup_nonce_check( $result ) {
	if ( ! strpos( $_SERVER['PHP_SELF'], 'sr-signup.php' ) ) {
		return $result;
	}
...
function wp_maybe_update_network_user_counts( $network_id = null ) {
	...
	/** This filter is documented in sr-includes/ms-functions.php */
...
function get_subdirectory_reserved_names() {
	 ...
	 *
	 * @since 3.0.0
	 * @since 4.4.0 'sr-admin', 'wp-content', 'sr-includes', 'wp-json', and 'embed' were added
