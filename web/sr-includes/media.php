 ...
 *
 * By default  adds `decoding="async"` to images but developers
...
function gallery_shortcode( $attr ) {
	...
	if ( apply_filters( 'use_default_gallery_style', ! $html5 ) ) {
		...
		<style{$type_attr}>
			...
			/* see gallery_shortcode() in sr-includes/media.php */
		</style>\n\t\t";
	}
 ...
 *     @type string $icon                  Icon URL of the attachment (e.g. /sr-includes/images/media/archive.png).
...
function wp_get_loading_optimization_attributes( $tag_name, $attr, $context ) {
	...
	if ( isset( $attr['loading'] ) ) {
		 ...
		 * only possible value that  core would apply on its own.
		 */
...
function wp_maybe_add_fetchpriority_high_attr( $loading_attrs, $tag_name, $attr ) {
	...
	if ( isset( $attr['fetchpriority'] ) ) {
		 ...
		 * is the only possible value that  core would apply on its
		 * own.
		 */
