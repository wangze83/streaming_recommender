...
/**
 * Retrieves the number of database queries during the  execution.
 *
 * @since 2.0.0
 *
 * @global wpdb $wpdb  database abstraction object.
...
function do_robots() {
	...
	$output  .= "Disallow: $path/sr-admin/\n";
	$output  .= "Allow: $path/sr-admin/admin-ajax.php\n";
...
function wp_unique_filename( $dir, $filename, $unique_filename_callback = null ) {
	...
	if ( $unique_filename_callback && is_callable( $unique_filename_callback ) ) {
		 ...
		 */
		if ( $is_image ) {
			/** This filter is documented in sr-includes/class-wp-image-editor.php */
...
function wp_upload_bits( $name, $deprecated, $bits, $time = null ) {
	...
	/** This filter is documented in sr-admin/includes/file.php */
...
/**
 * Loads custom DB error or display  DB error.
 *
	...
	if ( WP_DEBUG && apply_filters( 'doing_it_wrong_trigger_error', true, $function_name, $message, $version ) ) {
		if ( function_exists( '__' ) ) {
			...
			$message .= ' ' . sprintf(
				...
				__( '/documentation/article/debugging-in-wordpress/' )
			);
			...
			$message .= sprintf(
				...
				'/documentation/article/debugging-in-wordpress/'
			);
 ...
 *
 * Will remove sr-admin links to retrieve only return URLs not in the sr-admin
 * directory.
 *
 * @since 2.6.0
 *
...
function wp_guess_url() {
	if ( defined( 'WP_SITEURL' ) && '' !== WP_SITEURL ) {
		...
		if ( str_contains( $_SERVER['REQUEST_URI'], 'sr-admin' ) || str_contains( $_SERVER['REQUEST_URI'], 'sr-login.php' ) ) {
			$path = preg_replace( '#/(sr-admin/?.*|wp-login\.php.*)#i', '', $_SERVER['REQUEST_URI'] );
...
function wp_get_default_update_php_url() {
	return _x( '/support/update-php/', 'localized PHP upgrade information page' );
}
...
function wp_get_default_update_https_url() {
	...
	return __( '/documentation/article/why-should-i-use-https/' );
}
 ...
 *                                             started to load. Defaults to the value of
