 ...
 * generate rewrite rules for it. See /documentation/article/nginx/
 *
 * @package
 */
...
if ( is_admin() ) {
	// sr-admin pages are checked more carefully.
	if ( is_network_admin() ) {
		preg_match( '#/sr-admin/network/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches );
	} elseif ( is_user_admin() ) {
		preg_match( '#/sr-admin/user/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches );
	} else {
		preg_match( '#/sr-admin/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches );
	}
	...
	if ( '' === $pagenow || 'index' === $pagenow || 'index.php' === $pagenow ) {
		...
		if ( ! str_ends_with( $pagenow, '.php' ) ) {
			$pagenow .= '.php'; // For `Options +Multiviews`: /sr-admin/themes/index.php (themes.php is queried).
		}
	}
} else {
