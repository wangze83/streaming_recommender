...
function wp_check_invalid_utf8( $text, $strip = false ) {
	...
	// phpcs:ignore .PHP.NoSilencedErrors.Discouraged -- preg_match fails when it encounters invalid UTF8 in $text.
...
function sanitize_file_name( $filename ) {
	...
	if ( count( $parts ) <= 2 ) {
		/** This filter is documented in sr-includes/formatting.php */
...
function wp_trim_excerpt( $text = '', $post = null ) {
	...
	if ( '' === trim( $text ) ) {
		 ...
		 * which is generally used for the filter callback in  core.
		 */
		if ( $filter_removed ) {
		...
		case 'tag_base':
			...
			if ( 'permalink_structure' === $option && null === $error
				...
				$error = sprintf(
					...
					__( '/documentation/article/customize-permalinks/#choosing-your-permalink-structure' )
				);
			}
			break;
...
function wp_staticize_emoji_for_email( $mail ) {
	...
	/** This filter is documented in sr-includes/pluggable.php */
 ...
 *
 * @since 1.2.0
 * @since 4.4.0 Moved to sr-includes/formatting.php from sr-admin/includes/misc.php and added $length param.
 *
