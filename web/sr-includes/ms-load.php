...
function ms_load_current_site_and_network( $domain, $path, $subdomain = false ) {
	...
	if ( empty( $current_site ) ) {
		/** This action is documented in sr-includes/ms-settings.php */
		do_action( 'ms_network_not_found', $domain, $path );
	...
	if ( empty( $current_blog ) ) {
		...
		if ( $subdomain && ! defined( 'NOBLOGREDIRECT' ) ) {
			...
			$destination .= 'sr-signup.php?new=' . str_replace( '.' . $current_site->domain, '', $domain );
		} elseif ( $subdomain ) {
			/*
...
function ms_not_installed( $domain, $path ) {
	...
	if ( ! $wpdb->get_var( $query ) ) {
		$msg .= '<p>' . sprintf(
			...
			__( '<strong>Database tables are missing.</strong> This means that your host&#8217;s database server is not running,  was not installed properly, or someone deleted %s. You really should look at your database now.' ),
			'<code>' . $wpdb->site . '</code>'
		) . '</p>';
	} else {
		$msg .= '<p>' . sprintf(
	...
	$msg .= sprintf(
		...
		__( '/documentation/article/debugging-a-wordpress-network/' )
	);
