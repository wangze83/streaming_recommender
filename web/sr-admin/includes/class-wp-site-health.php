...
class WP_Site_Health {
	...
	public function show_site_health_tab( $tab ) {
		if ( 'debug' === $tab ) {
			require_once ABSPATH . 'sr-admin/site-health-info.php';
		}
	}
	...
	public function get_test_php_version() {
		...
		$result = array(
			...
			'description' => sprintf(
				...
				sprintf(
					...
					__( 'PHP is one of the programming languages used to build . Newer versions of PHP receive regular security updates and may increase your site&#8217;s performance. The minimum recommended version of PHP is %s.' ),
		 ...
		 * the expected minimum version that will be required by  in the near future.
		 */
		if ( $response['is_secure'] && $response['is_lower_than_future_minimum'] ) {
			...
			$result['label'] = sprintf(
				/* translators: %s: The server PHP version. */
				__( 'Your site is running on an outdated version of PHP (%s), which soon will not be supported by .' ),
				PHP_VERSION
			);
		...
		// No more security updates for the PHP version, and lower than the expected minimum version required by .
		if ( $response['is_lower_than_future_minimum'] ) {
			$message = sprintf(
				/* translators: %s: The server PHP version. */
				__( 'Your site is running on an outdated version of PHP (%s), which does not receive security updates and soon will not be supported by .' ),
				PHP_VERSION
			);
		} else {
	...
	/**
	 * Tests if the site can communicate with .org.
	 *
	 * @since 5.2.0
	 *
	 * @return array The test results.
	...
	public function get_test_is_in_debug_mode() {
		$result = array(
			...
			'actions'     => sprintf(
				...
				esc_url( __( '/documentation/article/debugging-in-wordpress/' ) ),
	...
	/**
	 * Tests if  can run automated background updates.
	 *
	...
	public function get_test_background_updates() {
		...
		if ( ! class_exists( 'WP_Site_Health_Auto_Updates' ) ) {
			require_once ABSPATH . 'sr-admin/includes/class-wp-site-health-auto-updates.php';
		}
	...
	public function get_test_update_temp_backup_writable() {
		...
		if ( ! $wp_content ) {
			$result['status']      = 'critical';
			$result['label']       = __( 'Unable to locate  content directory' );
			$result['description'] = sprintf(
	...
	public function get_test_rest_availability() {
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'The REST API is one way that  and other applications communicate with the server. For example, the block editor screen relies on the REST API to display and save your posts and pages.' )
			),
			'actions'     => '',
		...
		/** This filter is documented in sr-includes/class-wp-http-streams.php */
		...
		if ( is_wp_error( $r ) ) {
			...
			$result['description'] .= sprintf(
				...
				sprintf(
					// translators: 1: The  error code. 2: The HTTP status code error message.
					__( 'REST API Response: (%1$s) %2$s' ),
	...
	public function get_test_page_cache() {
		...
		$result = array(
			...
			'actions'     => sprintf(
				...
				__( '/documentation/article/optimization/#Caching' ),
	...
	public function get_test_persistent_object_cache() {
		...
		$action_url = apply_filters(
			...
			/* translators: Localized Support reference. */
			__( '/documentation/article/optimization/#persistent-object-cache' )
		);
		...
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'A persistent object cache makes your site&#8217;s database more efficient, resulting in faster load times because  can retrieve your site&#8217;s content and settings much more quickly.' )
			),
			'actions'     => sprintf(
				'<p><a href="%s" target="_blank" rel="noopener">%s<span class="screen-reader-text"> %s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a></p>',
				esc_url( $action_url ),
	...
	public static function get_tests() {
		 ...
		 * to complete should run asynchronously, to avoid extended loading periods within sr-admin.
		 *
		 * @since 5.2.0
	...
	public function can_perform_loopback() {
		...
		$url = site_url( 'sr-cron.php' );

		/*
		 * A post request is used for the sr-cron.php loopback test to cause the file
		 ...
		 * GET request  uses for sr-cron.php loopback requests but is close
	...
	public function wp_cron_scheduled_check() {
		// Bootstrap sr-admin, as WP_Cron doesn't do this for us.
		require_once trailingslashit( ABSPATH ) . 'sr-admin/includes/admin.php';
	...
	private function check_for_page_caching() {
		...
		return array(
			'advanced_cache_present'        => (
				...
				&&
				/** This filter is documented in sr-settings.php */
	...
	public function should_suggest_persistent_object_cache() {
		...
		$results = $wpdb->get_results(
			$wpdb->prepare(
				// phpcs:ignore .DB.PreparedSQL.InterpolatedNotPrepared -- This query cannot use interpolation.
