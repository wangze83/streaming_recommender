...
function wp_dashboard_setup() {
	...
	if ( 'POST' === $_SERVER['REQUEST_METHOD'] && isset( $_POST['widget_id'] ) ) {
		...
		ob_start(); // Hack - but the same hack sr-admin/widgets.php uses.
	...
	/** This action is documented in sr-admin/includes/meta-boxes.php */
...
function wp_dashboard_quick_press( $error_msg = false ) {
	...
	<form name="post" action="<?php echo esc_url( admin_url( 'post.php' ) ); ?>" method="post" id="quick-press" class="initial-form hide-if-no-js">
		...
		<div class="input-text-wrap" id="title-wrap">
			<label for="title">
				<?php
				/** This filter is documented in sr-admin/edit-form-advanced.php */
 ...
 * which is later replaced by Ajax call (see top of /sr-admin/index.php)
 *
 * @since 2.5.0
...
function wp_dashboard_primary() {
	$feeds = array(
		'news'   => array(
			 ...
			 */
			'link'         => apply_filters( 'dashboard_primary_link', __( '/news/' ) ),
			 ...
			 */
			'url'          => apply_filters( 'dashboard_primary_feed', __( '/news/feed/' ) ),
...
function wp_dashboard_php_nag() {
	...
	if ( isset( $response['is_secure'] ) && ! $response['is_secure'] ) {
		...
		if ( $response['is_lower_than_future_minimum'] ) {
			$message = sprintf(
				...
				__( 'Your site is running on an outdated version of PHP (%s), which does not receive security updates and soon will not be supported by . Ensure that PHP is updated on your server as soon as possible. Otherwise you will not be able to upgrade .' ),
				PHP_VERSION
			);
		} else {
			$message = sprintf(
		...
		$message = sprintf(
			/* translators: %s: The server PHP version. */
			__( 'Your site is running on an outdated version of PHP (%s), which soon will not be supported by . Ensure that PHP is updated on your server as soon as possible. Otherwise you will not be able to upgrade .' ),
			PHP_VERSION
		);
	} else {
		$message = sprintf(
	...
	<p>
		<?php _e( 'PHP is one of the programming languages used to build . Newer versions of PHP receive regular security updates and may increase your site&#8217;s performance.' ); ?>
		<?php
