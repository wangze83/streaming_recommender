...
function display_plugins_table() {
	...
	switch ( current_filter() ) {
		...
		case 'install_plugins_featured':
			printf(
				/* translators: %s: /plugins/ */
				'<p>' . __( 'Plugins extend and expand the functionality of . You may install plugins in the <a href="%s"> Plugin Directory</a> right from here, or upload a plugin in .zip format by clicking the button at the top of this page.' ) . '</p>',
				__( '/plugins/' )
			);
			break;
...
<div id="<?php echo $_tab; ?>-content" class='<?php echo $_with_banner; ?>'>
	<div class="fyi">
		<ul>
			...
			<?php } if ( ! empty( $api->slug ) && empty( $api->external ) ) { ?>
				<li><a target="_blank" href="<?php echo esc_url( __( '/plugins/' ) . $api->slug ); ?>/"><?php _e( '.org Plugin Page &#187;' ); ?></a></li>
				...
				<div class="counter-container">
						<span class="counter-label">
							...
							printf(
								...
								"/support/plugin/{$api->slug}/reviews/?filter={$key}",
								$aria_label,
	...
	foreach ( (array) $api->sections as $section_name => $content ) {
		$content = links_add_base_url( $content, '/plugins/' . $api->slug . '/' );
