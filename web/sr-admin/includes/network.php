		...
		if ( is_multisite() && get_site_option( 'ms_files_rewriting' ) ) {
			...
			$web_config_file .= '
                <rule name=" Rule 2" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . 'sr-admin$" ignoreCase="false" />
                    <action type="Redirect" url="' . $iis_subdir_replacement . 'sr-admin/" redirectType="Permanent" />
                </rule>
...
';
	...
	elseif ( $is_nginx ) : // End iis7_supports_permalinks(). Link to Nginx documentation instead:
		...
		printf(
			...
			__( '/documentation/article/nginx/' )
		);
		echo '</p></li>';
...
{$ms_files_rewriting}
# add a trailing slash to /sr-admin
RewriteRule ^{$subdir_match}sr-admin$ {$subdir_replacement_01}sr-admin/ [R=301,L]
