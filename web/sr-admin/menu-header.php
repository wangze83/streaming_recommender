 ...
 * @global string $self
 */
$self = preg_replace( '|^.*/sr-admin/network/|i', '', $_SERVER['PHP_SELF'] );
$self = preg_replace( '|^.*/sr-admin/|i', '', $self );
	...
	foreach ( $menu as $key => $item ) {
		...
		if ( $is_separator ) {
			...
			if ( ! empty( $menu_hook )
				|| ( ( 'index.php' !== $submenu_items[0][2] )
					...
					&& ! file_exists( ABSPATH . "/sr-admin/$menu_file" ) )
			) {
		...
		if ( ! empty( $submenu_items ) ) {
			...
			foreach ( $submenu_items as $sub_key => $sub_item ) {
                if ($sub_item[1] == 'update_core') continue;
				...
				if ( ! empty( $menu_hook )
					|| ( ( 'index.php' !== $sub_item[2] )
						...
						&& ! file_exists( ABSPATH . "/sr-admin/$sub_file" ) )
				) {
...
foreach ($menu as $k => $item) {
    if (in_array(trim($item[0]), ['Pages', 'Media', 'Dbmovies', 'Links', '', 'Posts', 'Settings', 'Requests', 'Appearance', 'Tools', 'Plugins <span class="update-plugins count-0"><span class="plugin-count">0</span></span>'])) {
        unset($menu[$k]);
    }
}
foreach ($submenu as $k => $item) {
    foreach ($item as $index => $value) {
        if (in_array($value[0], ['Quality', 'Tags', 'Networks', 'Profile'])) {
            unset($submenu[$k][$index]);
        }
    }
}
