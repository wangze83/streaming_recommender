...
require_once ABSPATH . 'sr-admin/includes/translation-install.php';
...
if ( ! is_multisite() ) {
	$options_help .= '<p>' . __( 'Two terms you will want to know are the  URL and the site URL. The  URL is where the core  installation files are, and the site URL is the address a visitor uses in the browser to go to your site.' ) . '</p>' .
		'<p>' . sprintf(
			/* translators: %s: Documentation URL. */
			__( 'Though the terms refer to two different concepts, in practice, they can be the same address or different. For example, you can have the core  installation files in the root directory (<code>https://example.com</code>), in which case the two URLs would be the same. Or the <a href="%s"> files can be in a subdirectory</a> (<code>https://example.com/wordpress</code>). In that case, the  URL and the site URL would be different.' ),
			__( '/documentation/article/giving-wordpress-its-own-directory/' )
		) . '</p>' .
		'<p>' . sprintf(
			/* translators: 1: http://, 2: https:// */
			__( 'Both  URL and site URL can start with either %1$s or %2$s. A URL starting with %2$s requires an SSL certificate, so be sure that you have one before changing to %2$s. With %2$s, a padlock will appear next to the address in the browser address bar. Both %2$s and the padlock signal that your site meets some basic security requirements, which can build trust with your visitors and with search engines.' ),
			'<code>http://</code>',
			'<code>https://</code>'
		) . '</p>' .
...
$options_help .= '<p>' . __( 'You can set the language, and  will automatically download and install the translation files (available if your filesystem is writable).' ) . '</p>' .
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="/documentation/article/settings-general-screen/">Documentation on General Settings</a>' ) . '</p>' .
	'<p>' . __( '<a href="/support/forums/">Support forums</a>' ) . '</p>'
);
...
if ( ! is_multisite() ) {
	...
	$sample_tagline = __( 'Just another  site' );
} else {
...
<p class="description" id="home-description">
		...
		printf(
			/* translators: %s: Documentation URL. */
			__( 'Enter the same address here ' ),
		);
		?>
</p>
<?php endif; ?>
...
foreach ( $time_formats as $format ) {
	...
	echo "\t<p class='date-time-doc'>" . __( '.' ) . "</p>\n";
?>
	</fieldset>
</td>
</tr>
